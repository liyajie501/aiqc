...
* 小商城前端，即aiqc-wx模块和renard-wx模块
  * 微信小程序
* 小商城后端，即aiqc-zj-api模块
  * Spring Boot 2.x
  * Spring MVC
...
1. 在aiqc-core模块的src/main/resources的application-core.yml资源文件中设置
    ```
    aiqc
        wx
            app-id: 开发者申请的app-id
...
2. 在aiqc-wx模块的project.config.json文件中设置
...
4. 建议开发者关闭当前项目或者直接关闭微信开发者工具，重新打开（因为此时aiqc-wx模块的appid可能未更新）。
   编译运行，尝试微信登录
...
1. 在aiqc-core-api模块的src/main/resources的application-core.yml资源文件中设置
    ...
    >    因此这就要求该地址是可访问的。
    > * 目前小商场后台服务的默认request mapping是`/zj/order/pay-notify`（见WxOrderController类的payNotify）,
    >    因此notify-url应该设置的地址类似于`http://www.example.com/zj/order/pay-notify`
...
2. 在aiqc-core-api模块的src/main/resources的application-core.yml资源文件中设置
...
小商城后端服务每一次请求都会检测是否存在HTTP头部域`X-aiqc-Token`。
...
注意
> aiqc-wx模块代码基于nideshop-mini-program的commit版本[acbf6276eb27abc6a48887cddd223d7261f0088e](https://github.com/tumobi/nideshop-mini-program/commit/acbf6276eb27abc6a48887cddd223d7261f0088e)。
> 由于改动变化较大，因此之后aiqc-wx将独立开发，不会合并nideshop-mini-program的更新。
...
```
// 本机开发时使用
var WxApiRoot = 'http://localhost:8082/zj/';
// 局域网测试使用
// var WxApiRoot = 'http://192.168.0.101:8082/zj/';
// 云平台部署时使用
//  var WxApiRoot = 'http://118.24.0.153:8082/zj/';
```
...
aiqc-wx模块采用storage来存储一些数据，以及支持组件间数据通信。
...
renard-wx是另外一个小程序前端，其后端API也是aiqc-zj-api。
