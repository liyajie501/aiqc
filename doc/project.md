...
aiqc是一个简单的商场系统，基于现有的开源项目，重新实现一个完整的前后端项目，包含小程序客户端、移动客户端和网页管理端。
...
* 基础系统子系统(platform)
  ...
  由数据库、aiqc-core模块、aiqc-db模块和aiqc-all模块组成;

* 小商场子系统(wxmall，即weixin mall)

  由aiqc-zj-api模块、aiqc-wx模块和renard-wx模块组成；

* 轻商城子系统(mobmall，即mobile mall)

  由aiqc-zj-api模块和aiqc-vue模块组成。
...
* 管理后台子系统(admin)
  ...
  由aiqc-admin-api模块和aiqc-admin模块组成。

而九个模块的开发设计到三种技术栈：

* Spring Boot技术栈

  采用IDEA开发工具，开发aiqc-core、aiqc-db、aiqc-admin-api、
  aiqc-zj-api和aiqc-all共五个模块；
  
* miniprogram（微信小程序）技术栈

  采用微信小程序开发工具，开发aiqc-wx模块和renard-wx模块；
  
* Vue技术栈

  采用VSC开发工具，开发aiqc-admin模块和aiqc-vue模块。
...
接下来，从项目的开发、部署（测试）和上线三个阶段介绍aiqc。
...
* MySQL数据访问地址`jdbc:mysql://localhost:3306/aiqc`
* aiqc-zj-api后端服务地址`http://localhost:8080/wx`，数据则来自MySQL
* aiqc-admin-api后端服务地址`http://localhost:8080/admin`,数据则来自MySQL
* aiqc-admin前端访问地址`http://localhost:9527`, 数据来自aiqc-admin-api
* aiqc-wx没有前端访问地址，而是直接在微信小程序工具上编译测试开发，最终会部署到微信官方平台（即不需要自己部署web服务器），而数据则来自aiqc-zj-api
...
2. 创建数据库、用户权限、数据库表和测试数据;
   数据库文件存放在aiqc-db/sql文件夹中，请开发者在MySQL中
   按照顺序运行以下脚本:
   * aiqc_schema.sql，用于创建数据库、用户和权限;
   * aiqc_table.sql，用于创建表;
   * aiqc_data.sql，用于导入测试数据。

注意：
> 建议采用命令行或者MySQL Workbench。如果采用Navicat可能导入失败。

如果开发者运行aiqc_schema.sql失败，可以打开该文件：
```
...
8. 采用Maven命令运行本项目的aiqc-all
   ...
   如果采用IDEA，则aiqc-all模块的Application类
   ...
   打开浏览器，输入
    ```
    http://localhost:8080/zj/index/index
    http://localhost:8080/admin/index/index
    ```
    如果出现JSON数据，则aiqc-all模块运行正常。
...
> 3. 开发者使用IDEA导入项目或者运行项目时可能会出现**软件卡顿**的现象，这通常是aiqc-admin的
>    node_modules文件夹内自动下载了大量的aiqc-admin的依赖库，当IDEA尝试索引该文件夹内的大量文件时
>    则出现IDEA卡顿的现象，具体解决方式可以参见[FAQ](./FAQ.md)

### 1.4.3 微信小程序开发环境

1. 安装微信小程序开发工具；
2. 导入本项目的aiqc-wx模块(或者renard-wx模块)文件夹；
3. 编译前，请确定aiqc-all模块已经运行，而aiqc-wx模块的config文件夹中的api.js已经设置正确的后端数据服务地址；
...
4. 请确定aiqc-all模块已经运行，然后点击`登录`，如果能够成功登录，则表明管理后台的前端和后端对接成功，运行正常。

本项目采用VSC（Visual Studio Code）开发aiqc-admin模块，开发者也可以采用其他熟悉的IDE。
...
1. 管理后台前端，即aiqc-admin模块，配置文件在aiqc-admin/config中，存在三个配置文件`dev.env.js`,`dep.env.js`
...
2. 小商场前端，即aiqc-wx模块，配置文件是`aiqc-zj/project.config.json`和`aiqc-zj/api.js`。
这里面最主要的配置信息是`project.config.json`中的`appid`，开发者需要设置自己申请的appid；
以及`apis.js`中的`WxApiRoot`，即小商场服务根地址。

    ```
    // 本机开发时使用
     var WxApiRoot = 'http://localhost:8080/zj/';
    // 局域网测试使用
    // var WxApiRoot = 'http://192.168.0.101:8080/zj/';
    // 云平台部署时使用
    // var WxApiRoot = 'http://118.24.0.153:8080/zj/';
    // 云平台上线时使用
    // var WxApiRoot = 'https://www.menethil.com.cn/zj/';

    ```

3. 管理后台后端和小商城后端，即多个Spring Boot模块，配置文件是每个模块的`aiqc-xx/src/main/java/resources`的
...
都是在`application-xx.yml`文件中，而`application.yml`文件仅仅用于引入其他模块的配置文件。
   ...
   例如aiqc-all模块的`application.yml`的内容是
   ```
    spring:
        profiles:
        ...
        message:
            encoding: UTF-8
    ```
    因此启动aiqc-all模块时，程序首先加载aiqc-all的`application.yml`，然后通过`spring.profiles.active`信息
...
如果开发者启动aiqc-all模块，则需要配置该模块的`application.yml`文件
```
logging:
  level:
    root:  ERROR
    ...
    org.mybatis:  ERROR
    com.sinosoft.aiqc.core:  ERROR
    com.sinosoft.aiqc.db:  ERROR
    com.sinosoft.aiqc.admin:  ERROR
    com.sinosoft.aiqc.wx:  ERROR
    com.sinosoft.aiqc:  ERROR
```

具体如何配置，请自行学习Spring Boot的日志配置。

`com.sinosoft.aiqc.core`定义aiqc-core模块的日志级别
`com.sinosoft.aiqc.db`定义aiqc-core模块的日志级别
`com.sinosoft.aiqc.wx`定义aiqc-zj-api模块的日志级别
`com.sinosoft.aiqc.admin`定义aiqc-admin-api模块的日志级别
`com.sinosoft.aiqc`而定义aiqc所有后端模块的日志级别

当然，如果开发者这里启动aiqc后端模块级别是DEBUG时，可能会发现并没有很多日志，
这是因为代码内部没有写很多日志，开发者可以根据需要添加。

注意：
> 如果开发者独立启动aiqc-zj-api模块，那么则需要配置aiqc-zj-api模块的
> `application.yml`文件来设置日志

#### 1.4.5.2 数据库连接配置

在aiqc-db模块的`application-db.yml`文件中配置数据库连接和druid：
...
spring:
  datasource:
    druid:
      url:  jdbc:mysql://localhost:3306/aiqc?useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC&allowPublicKeyRetrieval=true&verifyServerCertificate=false&useSSL=false
...
微信登录需要配置两个地方，
首先是小商场前端aiqc-wx模块（或renard-wx模块）中`project.config.json`文件的appid
其次是小商场后端aiqc-core模块的`application-core.yml`文件：
```bash
aiqc:
  wx:
...
注意
> 这里开发者可能会疑惑：小商场后端应该配置在aiqc-zj-api模块的`application-wx.yml`文件更合适。
> 这里放置在`application-core.yml`文件中是因为aiqc-core模块也依赖小程序appid配置信息。

#### 1.4.5.4 微信支付配置

在aiqc-core模块的`application-core.yml`文件中配置微信支付：
```
aiqc:
  wx:
    mch-id: 111111
    mch-key: xxxxxx
    notify-url: https://www.example.com/zj/order/pay-notify
```
...
aiqc-zj-api模块的WxOrderController类的payNotify方法所服务的API地址。

开发阶段可以采用一些技术实现临时外网地址映射本地，开发者可以百度关键字“微信 内网穿透”自行学习。

#### 1.4.5.5 邮件通知配置

在aiqc-core模块的`application-core.yml`文件中配置邮件通知服务：
```
aiqc:
  notify:
    mail:
...
验证配置成功：
当配置好信息以后，开发者可以运行aiqc-core模块的`MailTest`测试类，
独立发送邮件，然后登录邮箱查看邮件是否成功接收。

#### 1.4.5.6 短信通知配置

在aiqc-core模块的`application-core.yml`文件中配置短信通知服务：
```
aiqc:
  notify:
    # 短消息模版通知配置
...
验证配置成功：
当配置好信息以后，开发者可以aiqc-core模块的`SmsTest`测试类中设置手机号和
...
在aiqc-core模块的`application-core.yml`文件中配置微信通知服务：
```
aiqc:
  notify:
    # 微信模版通知配置
...
在aiqc-core模块的`application-core.yml`文件中配置快递鸟物流服务：
```
aiqc:
  notify:
  # 快鸟物流查询配置
...
验证配置成功：
当配置好信息以后，开发者可以aiqc-core模块的`ExpressTest`测试类中设置快递公司编码和
...
在aiqc-core模块的`application-core.yml`文件中配置对象存储服务：
...
aiqc:
  storage:
    ...
    local:
      ...
      address: http://localhost:8080/zj/storage/fetch/
```
...
其次，需要明确的是各模块之间的关系：
  ...
  * aiqc-zj-api模块会包含aiqc-core模块和aiqc-db模块，部署在服务器中
  * aiqc-admin-api模块会包含aiqc-core模块和aiqc-db模块，部署在服务器中
  * aiqc-all模块则会包装aiqc-zj-api模块和aiqc-admin-api模块；
  * aiqc-wx模块部署在微信开发者工具中，此外数据API地址指向aiqc-zj-api所在服务qi地址
  * aiqc-admin编译出的静态文件放在web服务器或者tomcat服务器，此外服务器地址设置指向3中aiqc-admin-api所在地址
  
最后，**如果项目部署云主机，则根据开发者的部署环境在以下文件中或代码中修改相应的配置。**

1. MySQL数据库设置合适的用户名和密码信息；
2. 后端服务模块设置合适的配置信息；
3. 小商场前端aiqc-wx模块`config/api.js`的`WxApiRoot`设置小商场后端服务的服务地址；
4. 管理后台前端aiqc-admin模块`config/dep.env.js`中的`BASE_API`设置管理后台后端服务的服务地址。

实际上，最终的部署方案是灵活的：

* 可以是同一云主机中安装一个Spring Boot服务，同时提供aiqc-admin、aiqc-admin-api和aiqc-zj-api三种服务
* 可以单一云主机中仅安装一个tomcat/nginx服务器部署aiqc-admin静态页面分发服务，
  然后部署两个Spring Boot的后端服务；
* 也可以把aiqc-admin静态页面托管第三方cdn，然后开发者部署两个后端服务
...
> 这里很可能是开发者aiqc-admin模块的`config/dep.env.js`或者`condig/prod.env.js`
...
打开浏览器，输入以下地址：
```
https://www.example.com/zj/index/index
...
上线之前需要修改代码或者配置文件：
1. aiqc-zj-api模块的WxOrderController类的payNotify方法的链接换成合适的地址。

   注意：
   > 换成什么地址都可以，但是这里不应该暴露出来。也就是说这个地址是微信商户平台
   > 和这里的小商场后端服务之间的交互API，对外公开会存在安全隐患。
   
2. aiqc-core模块需要配置application-core.yml
    ...
    aiqc:
        wx:
            ...
            mch-id: 111111
            mch-key: xxxxxx
            notify-url: http://www.example.com/zj/order/pay-notify
    ```
    
    这里的`aiqc.zj.notify-url`就是前面开发者自定义的地址。
    
3. aiqc-wx模块的`project.config.json`文件调整相应的值，
...
1. MySQL数据库设置合适的用户名和密码信息；
2. 管理后台后端服务模块设置合适的配置信息，建议开发者参考deploy/aiqc的外部配置文件，
   这样可以避免开发者对模块内部的开发配置文件造成修改；
3. 管理后台前端aiqc-admin模块`config/prod.env.js`中的`BASE_API`设置管理后台后端服务的服务地址。
...
aiqc-admin编译得到的前端文件在第一次加载时相当耗时，这里需要一些措施来优化启动速度
...
* deploy/.gitignore
* aiqc-admin/.gitignore
* aiqc-admin-api/.gitignore
* aiqc-core/.gitignore
* aiqc-db/.gitignore
* aiqc-zj-api/.gitignore
* aiqc-all/.gitignore
* .gitignore
...
2. 打包项目时，会编译打包项目相关模块到aiqc和db文件夹中；
...
但是存在一个问题，即多模块配置依赖。
例如，aiqc-db模块存在数据库配置信息，那么其他模块如何引入
aiqc-db模块的配置信息呢？

最简单的方式，就是其他模块把aiqc-db模块的配置信息拷贝到自己的
application配置文件中，但是问题就是数据库信息一旦改变则其他模块又要
再次手动修改，非常不方便。

目前本项目采用一种基于`spring.profiles.active`的方式，细节如下：
1. aiqc-db模块存在application.yml和application-db.yml两个配置文件，
    在application-db.yml配置文件中存放数据库配置信息；
2. aiqc-core模块也存在application.yml和application-core.yml两个配置文件,
    ...
    spring:
        profiles:
            active: core, db
    ```
    因此，如果单独启动aiqc-core模块，则会先读取application.yml配置文件，然后基于
    系统会根据`spring.profiles.active`读取application-db.yml和application-core.yml配置文件，
    因此就会自动读取aiqc-db模块的配置文件。
3. 以此类推，在aiqc-all模块中存在application.yml配置文件，其中内容是
    ```
    spring:
        profiles:
            active:  db, core, admin, wx
    ```
    因此，系统启动aiqc-all模块以后，则会先读取application.yml配置文件，然后基于
...
2. 测试或者部署阶段，系统打包成一个aiqc.jar二进制jar包，jar包内部配置文件是之前
    开发阶段的配置文件，此时在aiqc.jar的同级目录创建相同的配置文件，在这些配置文件则
    保存了测试或者部署阶段的配置信息。启动aiqc.jar时，系统会读取当前目录的配置文件，而
    不再读取jar包内部的配置文件。
3. 上线阶段，同样地，在aiqc.jar包同级目录创建上线配置文件。
...
* 6xx，小商城后端业务错误码，具体见aiqc-admin-api模块的`AdminResponseCode`类。
* 7xx，管理后台后端业务错误码，具体见aiqc-zj-api模块的`WxResponseCode`类。
