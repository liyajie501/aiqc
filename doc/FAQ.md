...
这里主要是指aiqc-zj-api、aiqc-wx和renard-wx三个模块的相关问题。
...
2. 开发者在aiqc-wx、renard-wx和aiqc-core模块的appid信息设置成自己申请的信息。
...
微信开发者工具中修改appid以后，请关闭aiqc-wx项目或者微信开发者工具，重新启动导入aiqc-wx。
...
把`WxOrderController.payNotify`所代表的本地地址，例如`http://localhots/zj/order/pay-notify`,
转换成外网可以访问的地址，例如`http://xxx.com/zj/order/pay-notify`，最后
...
这里主要是指aiqc-admin-api和aiqc-admin两个模块的相关问题。
...
2. 其次，需要明白报错的地方，是aiqc-admin/src/utils/request.js文件中;
...
这里主要是指aiqc-db、aiqc-core和aiqc-all模块三个模块的相关问题。
...
```
org.apache.ibatis.binding.BindingException: Invalid bound statement (not found): com.sinosoft.aiqc.db.dao.XXXX
```
...
The error may exist in com/sinosoft/aiqc/db/dao/aiqcSystemMapper.xml
The error may involve com.sinosoft.aiqc.db.dao.aiqcSystemMapper.selectByExample-Inline
The error occurred while setting parameters
SQL: select id, key_name, key_value, add_time, update_time, deleted from aiqc_system
...
开发者直接（或使用Navicat）运行aiqc_schema.sql时运行失败。
...
首先，请开发者请直接打开aiqc_schema.sql文件，可以看到
```
...
因此，如果开发者运行aiqc_schema.sql失败，开发者可以自行使用
相关SQL命令或者使用SQL工具创建数据库、用户和分配权限工作。

此外实际上，**开发者也不应该在部署或者上线阶段运行aiqc_schema.sql**
...
应该是aiqc-admin模块的node_modules文件夹导致的。
node_modules是aiqc-admin所依赖的项目库，可能有近200M的文件。
